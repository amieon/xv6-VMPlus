...
user/_cow_bidir:     file format elf64-littleriscv
...
main(void)
{
  ...
  2a:	8ea50513          	addi	a0,a0,-1814 # 910 <malloc+0xdc>
  2e:	74c000ef          	jal	ra,77a <printf>
    exit(0);
  32:	4501                	li	a0,0
  34:	2ea000ef          	jal	ra,31e <exit>
  if(pid < 0) exit(1);
  38:	4505                	li	a0,1
  3a:	2e4000ef          	jal	ra,31e <exit>
    }
  ...
  wait(0);
  4e:	4501                	li	a0,0
  50:	2d6000ef          	jal	ra,326 <wait>
  printf("parent: x=%d (expect 300)\n", x);
  54:	408c                	lw	a1,0(s1)
  56:	00001517          	auipc	a0,0x1
  5a:	8ca50513          	addi	a0,a0,-1846 # 920 <malloc+0xec>
  5e:	71c000ef          	jal	ra,77a <printf>
  if(x != 300) printf("FAIL parent\n");
  62:	409c                	lw	a5,0(s1)
  64:	01279b63          	bne	a5,s2,7a <main+0x7a>
  printf("PASS\n");
  68:	00001517          	auipc	a0,0x1
  6c:	8e850513          	addi	a0,a0,-1816 # 950 <malloc+0x11c>
  70:	70a000ef          	jal	ra,77a <printf>
  exit(0);
  74:	4501                	li	a0,0
  76:	2a8000ef          	jal	ra,31e <exit>
  if(x != 300) printf("FAIL parent\n");
  7a:	00001517          	auipc	a0,0x1
  7e:	8c650513          	addi	a0,a0,-1850 # 940 <malloc+0x10c>
  82:	6f8000ef          	jal	ra,77a <printf>
...
stat(const char *n, struct stat *st)
{
 ...
 1d8:	84aa                	mv	s1,a0
  ...
  r = fstat(fd, st);
 1da:	85ca                	mv	a1,s2
 1dc:	19a000ef          	jal	ra,376 <fstat>
 1e0:	892a                	mv	s2,a0
  close(fd);
 1e2:	8526                	mv	a0,s1
 1e4:	162000ef          	jal	ra,346 <close>
  return r;
}
 1e8:	854a                	mv	a0,s2
...
printint(int fd, long long xx, int base, int sgn)
{
 ...
 43e:	52650513          	addi	a0,a0,1318 # 960 <digits>
 442:	883e                	mv	a6,a5
...
vprintf(int fd, const char *fmt, va_list ap)
{
 ...
 4fe:	466c8c93          	addi	s9,s9,1126 # 960 <digits>
 ...
 722:	23a90913          	addi	s2,s2,570 # 958 <malloc+0x124>
        for(; *s; s++)
...
malloc(uint nbytes)
{
 ...
 8be:	74a73b23          	sd	a0,1878(a4) # 1010 <freep>
      return (void*)(p + 1);  /* 返回实际数据区域的指针 */
 ...
 8e6:	ec7ff0ef          	jal	ra,7ac <free>
  return freep;
 8ea:	00093503          	ld	a0,0(s2)
      if((p = morecore(nunits)) == 0)
 8ee:	dd61                	beqz	a0,8c6 <malloc+0x92>
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 8f0:	611c                	ld	a5,0(a0)
    if(p->s.size >= nunits){  /* 找到足够大的块 */
 8f2:	4798                	lw	a4,8(a5)
 8f4:	fa9778e3          	bgeu	a4,s1,8a4 <malloc+0x70>
    if(p == freep)
 8f8:	00093703          	ld	a4,0(s2)
 8fc:	853e                	mv	a0,a5
 8fe:	fef719e3          	bne	a4,a5,8f0 <malloc+0xbc>
  p = sbrk(nu * sizeof(Header));  /* 调用sbrk扩展堆 */
 902:	8552                	mv	a0,s4
 904:	9e7ff0ef          	jal	ra,2ea <sbrk>
  if(p == SBRK_ERROR)  /* 检查是否分配失败 */
 908:	fd551ce3          	bne	a0,s5,8e0 <malloc+0xac>
        return 0;  /* 内存分配失败 */
 90c:	4501                	li	a0,0
 90e:	bf65                	j	8c6 <malloc+0x92>
